...
You can obtain your gene references from GenBank or the multi-FASTA files generated by GeSeq above. All reference sequences should be in a single fasta file `gene_ref.fas`. It is recommended that for diverse groups, you use **more than one** reference sequences for each gene to catch the genetic variance. 
...
#### 4). Intergenic regions and custom reference from GenBank file
